...
find . -name '*.o' -delete
find . -name '*.so' -delete
find . -name '*.cpp' -delete  # if applicable
...
if [[ "$build_framework" == true ]]; then
  ...
  if [[ ${WITH_EDITABLE} == true ]]; then
    ...
    USE_CYTHON=1 uv pip install --no-build-isolation --editable . --config-setting='editable_mode=compat' || exit
  else
    # uv pip install --no-build-isolation --config-setting='--build-option=--use-cython' . || exit
    USE_CYTHON=1 uv pip install --no-build-isolation . || exit
  fi
fi
